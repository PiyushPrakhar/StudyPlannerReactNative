 ...
 */
public class PremiumPlannerAI extends UiManagingDelegate {
    public static final String SKU_ID = "premium_planner_ai";

    public PremiumPlannerAI(BillingProvider billingProvider) {
    ...
    public void onBindViewHolder(SkuRowData data, RowViewHolder holder) {
        ...
        int textId = mBillingProvider.isPlannerAiSubscribed() ? R.string.button_own
                : R.string.button_buy;
        holder.button.setText(textId);
        holder.skuIcon.setImageResource(R.drawable.planner);
    }

    @Override
    public void onButtonClicked(SkuRowData data) {
        if (data != null && mBillingProvider.isPlannerAiSubscribed()) {
